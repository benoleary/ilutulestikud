...
// ReadOnly defines the interface for structs which should encapsulate the state of a
// player which can be read but not written.
...
// implementing the ReadOnly interface out of instances of endpoint.PlayerState, which
// normally represents de-serialized JSON from the frontend, and for tracking the objects
// by their identifier, which is the player name encoded in a way compatible with the
// identifier being a segment of a URI with '/' as a delimiter. It is also responsible
...
type Collection interface {
	...
	// the ReadOnly interface with information given by the endpoint.PlayerState object,
	// and return the identifier of the newly-created player, along with an error which
	// of course should be nil if there was no problem. It should return an error if the
	...
	// a nil ReadOnly.
	Get(playerIdentifier string) (ReadOnly, error)
...
func RegisteredPlayersForEndpoint(collection Collection) endpoint.PlayerList {
